...
.controller('ListviewCtrl', function($scope, $http, SERVER) {
  ...
  function loadTopics(params, callback){
    ...
    $scope.doRefresh = function(){
      var params = {'after': $scope.topics[0].tarefa_track};
...
.controller('DetailCtrl', function($scope, $http, $stateParams, SERVER){
  $http.post(SERVER.url+'/detail', {tarefa_track: $stateParams.id})
    .success(function(response){      
     ...
     $scope.loadInteracoes = function(topic)
    {
      ...
      .success(function(response){
        console.log(response);
                 $scope.list_interacoes = response;
      });
    }
